...
const UserInfo = () => {
    ...
    const items = [
        ...
        { id: 3, icon: <FiUserPlus className="icon" />, label: 'following', value: following, color: 'pink' },
    ...
    return (
        <section className="section">
            <Wrapper className="section-center">
                {items.map(item => {
                    return <Item id={item.id} icon={item.icon} label={item.label} value={item.value} color={item.color}/>;
                })}
            </Wrapper>
        </section>
    );
